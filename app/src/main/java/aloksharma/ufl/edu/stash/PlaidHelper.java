...
//                Log.d("StashLog", "Size: " + accessTokensEncrypted.size());
    ...
    private Double plaidPostRequest(String plaidUrl, List<NameValuePair>
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                String bankName = jObject.getJSONArray("accounts")
                        .getJSONObject
    ...
    public Response getMFAQuestion(String username, String password, String
            bankName) {
        List<NameValuePair> postArgs = new ArrayList();
        Log.d("StashPlaidHelper", bankName);
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 201) {
                ...
                return new Response(response.getStatusLine()
                        .getStatusCode(), access_token, jObject.getJSONArray
                        ("mfa").getJSONObject(0).get("question").toString());
            }
        } catch (ClientProtocolException e) {
            e.printStackTrace();
        } catch (IOException e) {
    ...
    public Response postMFAAnswer(String username, String password, String
            type, String access_token, String answer) {
        List<NameValuePair> postArgs = new ArrayList();
        Log.d("StashPlaidHelper", type);

        String plaidUrl = context.getString(R.string.plaid_mfa_url);

        postArgs.add(new BasicNameValuePair("username", username));
        postArgs.add(new BasicNameValuePair("password", password));
        postArgs.add(new BasicNameValuePair("type", type));
        postArgs.add(new BasicNameValuePair("access_token", access_token));
        postArgs.add(new BasicNameValuePair("mfa", answer));
        HttpClient httpClient = new DefaultHttpClient();
        HttpPost httpPost = new HttpPost(plaidUrl);
        ...
        try {
            HttpResponse response = httpClient.execute(httpPost);
            if (response.getStatusLine().getStatusCode() == 201) {
                Log.d("postMFA", "StatusCode 201");
                String responseString = EntityUtils.toString(response
                        .getEntity());
                JSONObject jObject = new JSONObject(responseString);
                Log.d("postMFA", "PLAID RESPONSE: " + responseString);
                ...
                access_token = jObject.getString("access_token");
                AesCbcWithIntegrity.CipherTextIvMac cipherTextIvMac =
                        AesCbcWithIntegrity.encrypt(access_token,
                                generateNewKeys());
                String access_token_encrypted = cipherTextIvMac.toString();
                ...
                Log.d("postMFA, avai balance", String.valueOf(availableBalance));
                sharedPrefEditor.putString("balance", availableBalance + "");
                sharedPrefEditor.commit();
                return new Response(response.getStatusLine().getStatusCode(), availableBalance);
            }
        } catch (ClientProtocolException e) {
            e.printStackTrace();
        } catch (IOException e) {
    ...
    public class Response {
        ...
        public Response(int statusCode, String access_token, String s) {
            responseCode = statusCode;
            this.access_token = access_token;
            question = s;
        }

        public Response(int statusCode, Double balance) {
            responseCode = statusCode;
